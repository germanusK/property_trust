    ...
    <div class="w-full">
        ...
        <div class="h-96 w-full mb-3 overflow-y-scroll no-scrollbar">
            ...
            <table class="w-full">
                ...
                <tbody>
                    @php($k = 1)
                    @forelse(\App\Models\Category::all() as $value)
                    <tr class="py-2 border-y mb-1 bg-slate-700 bg-opacity-20 border-cyan-700 text-slate-200 divide-x divide-slate-600">
                        ...
                        <td class="text-base px-2 flex">
                            <a href=" {{ route('rest.categories.edit', $value['id'])}} "><span title="edit" class="text-base text-sky-300 mx-2 fas fa-edit"></span></a>
                            <form action=" {{route('rest.categories.delete', $value['id'])}} " method="post">{{ csrf_field() }}<button type="submit"><span title="delete" class="text-base text-red-400 mx-2 fas fa-trash"></span></button></form>
                        </td>
                    </tr>
                    @empty
                    <div class="flex w-full break-words py-1 text-center mt-4 border-y text-slate-100 border-slate-100 align-middle items-stretch justify-between text-xl">
                        No categories available yet. Consider adding some categories
                    </div> 
                    @endforelse
                </tbody>
            </table>

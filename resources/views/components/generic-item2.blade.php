<div class="sm:max-w-xs md:max-w-sm h-auto px-3 py-6" style="min-width: 18.5rem;">
    <div class="prop w-full h-auto bg-stone-900 rounded-lg border-4 border-stone-300 hover:border-gray-600 border-double relative">
        <div class="w-full h-96 p-2 rounded-lg items-center align-middle justify-center flex relative">
            ...
            <img src="{{ asset( $data->images->first()->img_path ?? null) }}" alt="failed to load image " class="w-full h-full object-center max-h-full object-cover overflow-hidden rounded-t-lg">
            <div class="w-full opacity-75 absolute bottom-0">
                <div class="w-full h-auto py-2 border-t-2 border-white rounded-t-xl rounded-b-lg bg-black hover:bg-blue-900 mt-2">
                    <div class="text-slate-50 text-base font-semibold text-center text-ellipsis py-1 line-clamp-2"> {{$data['name']}} </div>
                    <div class="text-slate-100 text-base text-center line-clamp-2"> {!! $data['description'] !!} </div>
                </div>
